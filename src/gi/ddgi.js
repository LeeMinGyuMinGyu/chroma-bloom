...
export class DDGI {
  constructor(renderer, scene, camera, { enabled = false } = {}) {
    ...
    this.active = true;   // 방1에서만 true (roomManager가 제어)
    ...
    document.addEventListener('keydown', (e) => {
      if (e.code === 'KeyG' && this.debugMesh) {
        ...
        this.debugMesh.visible = this.active && this.debugVisible;
      }
  ...
  // 방 전환용: 방1에서만 DDGI(프로브 점 + 주입) 표시. 방2·3에선 끔.
  setActive(on) {
    this.active = on;
    if (this.debugMesh) this.debugMesh.visible = on && this.debugVisible;
    if (this.uInject) this.uInject.value = on ? 1.0 : 0.0;
  }
  ...
  update() {
    if (!this.enabled || !this.giReady || !this.active) return;
    try {
